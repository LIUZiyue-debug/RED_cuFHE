...
void adder_16(Ctxt *sum, Ctxt *a, Ctxt *b, Stream *st) {
    static Ctxt G[6][16];
    static Ctxt P[6][16];
    constexpr int inner_threads = 16 * 2;
    #pragma omp parallel
    {
        ...
        for (int k = 1; k < 6; k++) {
            ...
            for (int i = 0; i < 16; i++) {
                if (i >= distance) {
                    ...
                    And(P[k][i], P[k - 1][i], P[k - 1][i - distance], st[i + 16]);
                } else {
                    Copy(G[k][i], G[k - 1][i], st[i]);
                    Copy(P[k][i], P[k - 1][i], st[i + 16]);
                }
                if (i == 15) Synchronize();
            }
            Synchronize();
...
void multiply_16(Ctxt *product, const Ctxt *a, const Ctxt *b, Stream *st) {
    static Ctxt shifted_temp[16][16];
    constexpr int inner_threads = 16;
    #pragma omp parallel for schedule(static) num_threads(inner_threads)
    for (int idx = 0; idx < 16 * 16; idx++) {
        And(shifted_temp[idx / 16][idx % 16], a[idx % 16], b[idx / 16], st[idx]);
    }
    Synchronize();
    #pragma omp parallel for schedule(static) num_threads(inner_threads)
    for (int idx = 16; idx < 16 * 16; idx++) {
        if (idx % 16 < idx / 16) {
            Copy(shifted_temp[idx / 16][idx % 16], bit_0, st[idx % 16]);
        } else {
            Copy(shifted_temp[idx / 16][idx % 16], shifted_temp[idx / 16][idx % 16 - idx / 16], st[idx % 16]);
        }
    }
    Synchronize();
    omp_set_nested(1);
...
void subtractor_16(Ctxt *diff, Ctxt *a, Ctxt *b, Stream *st) {
    static Ctxt G[6][16];
    static Ctxt P[6][16];
    static Ctxt b_neg[16];
    constexpr int inner_threads = 16 * 2;
    #pragma omp parallel
    {
        #pragma omp parallel for schedule(static) num_threads(inner_threads)
...
void vectorSubtractor(Ctxt result[3][16], Ctxt a[3][16], Ctxt b[3][16], Stream *st) {
    for (int i = 0; i < 3; ++i) {
        subtractor_16(result[i], a[i], b[i], st);
    }
}

void vectorCross(Ctxt result[3][16], Ctxt a[3][16], Ctxt b[3][16], Stream *st) {
    Ctxt temp1[16], temp2[16];
    multiply_16(temp1, a[1], b[2], st);
    multiply_16(temp2, a[2], b[1], st);
    subtractor_16(result[0], temp1, temp2, st);
    multiply_16(temp1, a[2], b[0], st);
    multiply_16(temp2, a[0], b[2], st);
    subtractor_16(result[1], temp1, temp2, st);
    multiply_16(temp1, a[0], b[1], st);
    multiply_16(temp2, a[1], b[0], st);
    subtractor_16(result[2], temp1, temp2, st);
}

void vectorDot(Ctxt result[16], Ctxt a[3][16], Ctxt b[3][16], Stream *st) {
    Ctxt partial[16];
    for (int i = 0; i < 16; i++) Copy(result[i], bit_0);
    for (int i = 0; i < 3; ++i) {
        multiply_16(partial, a[i], b[i], st);
        adder_16(result, result, partial, st);
    }
}

void computeUVAndDet(Ctxt O[3][16], Ctxt D[3][16], Ctxt V0[3][16], Ctxt V1[3][16], Ctxt V2[3][16], Ctxt u_plus_v[16], Ctxt det[16], Stream *st) {
    Ctxt E1[3][16], E2[3][16];
    Ctxt P[3][16], T[3][16], Q[3][16];
    Ctxt u[16], v[16];
    ...
    vectorDot(v, D, Q, st);
    adder_16(u_plus_v, u, v, st);
}

void Add16BitEncrypted(Ctxt *sum, Ctxt *A, Stream *st) {
    int n = 16;
    Ctxt* tempSum = new Ctxt[n];
    Ctxt* tempCarry = new Ctxt[n];
    Ctxt* newCarry = new Ctxt[n];
    for (int i = 0; i < n; ++i) {
        Xor(tempSum[i], A[i], A[i + 16], st[i]);
        And(tempCarry[i], A[i], A[i + 16], st[i]);
    }
    Synchronize();
...
void server(int shares, uint16_t kNumSMs, int idx, Ctxt** answers, Ctxt* final_answer, Stream** st) {
    while(1) {
        cout << 1 << endl;
...
void setup(uint16_t kNumSMs, Ctxt** inputs, Ptxt** pt, Stream** st, int idx) {
    cudaSetDevice(idx);
    Initialize(bk);
...
int main(){
    ...
    Stream* st;
    Ptxt O_p[3][16];
    Ptxt D_p[3][16], V0_p[3][16], V1_p[3][16], V2_p[3][16];
    Ctxt u_plus_v[16];
    Ctxt det[16];
    SetSeed();
    PriKeyGen(pri_key);
    ...
    Ctxt O[3][16];
    Ctxt D[3][16], V0[3][16], V1[3][16], V2[3][16];
    ...
    for (int i = 1; i < (num_threads+1); i++) {
        if (i != 0) {
            ...
            adder_16(det, O[0], D[0], st);
            t2 = high_resolution_clock::now();

            //multiply_16(u_plus_v, O[0], D[0], st);
    ...
    Ptxt* recovered_u_plus_v = new Ptxt[16];
    Ptxt* recovered_det = new Ptxt[16];
