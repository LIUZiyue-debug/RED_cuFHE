...
__global__ void adder_32(Ctxt *sum, Ctxt *a, Ctxt *b, Stream *st) {
    __shared__ Ctxt G[6][32];
    __shared__ Ctxt P[6][32];
    int i = threadIdx.x;
    ...
    for (int k = 1; k < 6; k++) {
        ...
        if (i < 32) {
            if (i >= distance) {
                Ctxt temp_G, temp_P;
                And(temp_G, P[k - 1][i], G[k - 1][i - distance], st[i]);
                Or(G[k][i], G[k - 1][i], temp_G, st[i]);
                And(temp_P, P[k - 1][i], P[k - 1][i - distance], st[i + 32]);
                Copy(P[k][i], temp_P, st[i + 32]);
            } else {
...
void multiply_32(Ctxt *product, const Ctxt *a, const Ctxt *b, Stream *st) {
    ...
    constexpr int inner_threads = 32;
...
void subtractor_32(Ctxt *diff, Ctxt *a, Ctxt *b, Stream *st) {
    ...
    constexpr int inner_threads = 32 * 2;
    #pragma omp parallel
    {
        #pragma omp parallel for schedule(static) num_threads(inner_threads)
        for (int i = 0; i < 32; i++) {
...
int main(){
    ...
    for (int i = 1; i < (num_threads+1); i++) {
        if (i != 0) {
            ...
            dim3 blockSize(32); 
            dim3 gridSize(1);
            Ctxt *sum_dev, *a_dev, *b_dev;
            Stream *st_dev;
            cudaMalloc(&sum_dev, 32 * sizeof(Ctxt));
            cudaMalloc(&a_dev, 32 * sizeof(Ctxt));
            cudaMalloc(&b_dev, 32 * sizeof(Ctxt));
            cudaMalloc(&st_dev, 2048 * sizeof(Stream));
            
            cudaMemcpy(a_dev, O[0], 32 * sizeof(Ctxt), cudaMemcpyHostToDevice);
            cudaMemcpy(b_dev, D[0], 32 * sizeof(Ctxt), cudaMemcpyHostToDevice);
            cudaMemcpy(sum_dev, st, 32 * sizeof(Stream), cudaMemcpyHostToDevice);
            adder_32<<<gridSize, blockSize>>>(det, a_dev, b_dev, st_dev);
            
            
            //adder_32(det, O[0], D[0], st);
            t2 = high_resolution_clock::now();

            //multiply_32(u_plus_v, O[0], D[0], st);
