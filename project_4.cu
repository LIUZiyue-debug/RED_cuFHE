...
void adder_4(Ctxt *sum, Ctxt *a, Ctxt *b, Stream *st) {
    static Ctxt G[6][4];
    static Ctxt P[6][4];
    constexpr int inner_threads = 4 * 2;
    #pragma omp parallel
    {
        ...
        for (int k = 1; k < 6; k++) {
            ...
            for (int i = 0; i < 4; i++) {
                if (i >= distance) {
                    ...
                    And(P[k][i], P[k - 1][i], P[k - 1][i - distance], st[i + 4]);
                } else {
                    Copy(G[k][i], G[k - 1][i], st[i]);
                    Copy(P[k][i], P[k - 1][i], st[i + 4]);
                }
                if (i == 15) Synchronize();
            }
            Synchronize();
...
void multiply_4(Ctxt *product, const Ctxt *a, const Ctxt *b, Stream *st) {
    static Ctxt shifted_temp[4][4];
    constexpr int inner_threads = 4;
    #pragma omp parallel for schedule(static) num_threads(inner_threads)
    for (int idx = 0; idx < 4 * 4; idx++) {
        And(shifted_temp[idx / 4][idx % 4], a[idx % 4], b[idx / 4], st[idx]);
    }
    Synchronize();
    #pragma omp parallel for schedule(static) num_threads(inner_threads)
    for (int idx = 4; idx < 4 * 4; idx++) {
        if (idx % 4 < idx / 4) {
            Copy(shifted_temp[idx / 4][idx % 4], bit_0, st[idx % 4]);
        } else {
            Copy(shifted_temp[idx / 4][idx % 4], shifted_temp[idx / 4][idx % 4 - idx / 4], st[idx % 4]);
        }
    }
    Synchronize();
    omp_set_nested(1);
...
void subtractor_4(Ctxt *diff, Ctxt *a, Ctxt *b, Stream *st) {
    static Ctxt G[6][4];
    static Ctxt P[6][4];
    static Ctxt b_neg[4];
    constexpr int inner_threads = 4 * 2;
...
void vectorSubtractor(Ctxt result[3][4], Ctxt a[3][4], Ctxt b[3][4], Stream *st) {
    for (int i = 0; i < 3; ++i) {
        subtractor_4(result[i], a[i], b[i], st);
    }
}

void vectorCross(Ctxt result[3][4], Ctxt a[3][4], Ctxt b[3][4], Stream *st) {
    Ctxt temp1[4], temp2[4];
    multiply_4(temp1, a[1], b[2], st);
    multiply_4(temp2, a[2], b[1], st);
    subtractor_4(result[0], temp1, temp2, st);
    multiply_4(temp1, a[2], b[0], st);
    multiply_4(temp2, a[0], b[2], st);
    subtractor_4(result[1], temp1, temp2, st);
    multiply_4(temp1, a[0], b[1], st);
    multiply_4(temp2, a[1], b[0], st);
    subtractor_4(result[2], temp1, temp2, st);
}

void vectorDot(Ctxt result[4], Ctxt a[3][4], Ctxt b[3][4], Stream *st) {
    Ctxt partial[4];
    for (int i = 0; i < 4; i++) Copy(result[i], bit_0);
    for (int i = 0; i < 3; ++i) {
        multiply_4(partial, a[i], b[i], st);
        adder_4(result, result, partial, st);
    }
}

void computeUVAndDet(Ctxt O[3][4], Ctxt D[3][4], Ctxt V0[3][4], Ctxt V1[3][4], Ctxt V2[3][4], Ctxt u_plus_v[4], Ctxt det[4], Stream *st) {
    Ctxt E1[3][4], E2[3][4];
    Ctxt P[3][4], T[3][4], Q[3][4];
    Ctxt u[4], v[4];
    ...
    vectorDot(v, D, Q, st);
    adder_4(u_plus_v, u, v, st);
}

void Add4BitEncrypted(Ctxt *sum, Ctxt *A, Stream *st) {
    int n = 4;
    Ctxt* tempSum = new Ctxt[n];
    Ctxt* tempCarry = new Ctxt[n];
    Ctxt* newCarry = new Ctxt[n];
    for (int i = 0; i < n; ++i) {
        Xor(tempSum[i], A[i], A[i + 4], st[i]);
        And(tempCarry[i], A[i], A[i + 4], st[i]);
    }
    Synchronize();
...
int main(){
    ...
    Stream* st;
    Ptxt O_p[3][4];
    Ptxt D_p[3][4], V0_p[3][4], V1_p[3][4], V2_p[3][4];
    Ctxt u_plus_v[4];
    Ctxt det[4];
    SetSeed();
    PriKeyGen(pri_key);
    ...
    Ctxt O[3][4];
    Ctxt D[3][4], V0[3][4], V1[3][4], V2[3][4];
    ...
    for (int i = 1; i < (num_threads+1); i++) {
        if (i != 0) {
            ...
            adder_4(det, O[0], D[0], st);
            int a = rand(), b = rand();
            t2 = high_resolution_clock::now();
            int c = a * b;
            //multiply_4(u_plus_v, O[0], D[0], st);
    ...
    Ptxt* recovered_u_plus_v = new Ptxt[4];
    Ptxt* recovered_det = new Ptxt[4];
