...
void adder_8(Ctxt *sum, Ctxt *a, Ctxt *b, Stream *st) {
    static Ctxt G[6][8];
    static Ctxt P[6][8];
    constexpr int inner_threads = 8 * 2;
    #pragma omp parallel
    {
        ...
        for (int k = 1; k < 6; k++) {
            ...
            for (int i = 0; i < 8; i++) {
                if (i >= distance) {
                    ...
                    And(P[k][i], P[k - 1][i], P[k - 1][i - distance], st[i + 8]);
                } else {
                    Copy(G[k][i], G[k - 1][i], st[i]);
                    Copy(P[k][i], P[k - 1][i], st[i + 8]);
                }
                if (i == 15) Synchronize();
            }
            Synchronize();
...
void multiply_8(Ctxt *product, const Ctxt *a, const Ctxt *b, Stream *st) {
    static Ctxt shifted_temp[8][8];
    constexpr int inner_threads = 8;
    #pragma omp parallel for schedule(static) num_threads(inner_threads)
    for (int idx = 0; idx < 8 * 8; idx++) {
        And(shifted_temp[idx / 8][idx % 8], a[idx % 8], b[idx / 8], st[idx]);
    }
    Synchronize();
    #pragma omp parallel for schedule(static) num_threads(inner_threads)
    for (int idx = 8; idx < 8 * 8; idx++) {
        if (idx % 8 < idx / 8) {
            Copy(shifted_temp[idx / 8][idx % 8], bit_0, st[idx % 8]);
        } else {
            Copy(shifted_temp[idx / 8][idx % 8], shifted_temp[idx / 8][idx % 8 - idx / 8], st[idx % 8]);
        }
    }
    Synchronize();
    omp_set_nested(1);
...
void subtractor_8(Ctxt *diff, Ctxt *a, Ctxt *b, Stream *st) {
    static Ctxt G[6][8];
    static Ctxt P[6][8];
    static Ctxt b_neg[8];
    constexpr int inner_threads = 8 * 2;
...
void vectorSubtractor(Ctxt result[3][8], Ctxt a[3][8], Ctxt b[3][8], Stream *st) {
    for (int i = 0; i < 3; ++i) {
        subtractor_8(result[i], a[i], b[i], st);
    }
}

void vectorCross(Ctxt result[3][8], Ctxt a[3][8], Ctxt b[3][8], Stream *st) {
    Ctxt temp1[8], temp2[8];
    multiply_8(temp1, a[1], b[2], st);
    multiply_8(temp2, a[2], b[1], st);
    subtractor_8(result[0], temp1, temp2, st);
    multiply_8(temp1, a[2], b[0], st);
    multiply_8(temp2, a[0], b[2], st);
    subtractor_8(result[1], temp1, temp2, st);
    multiply_8(temp1, a[0], b[1], st);
    multiply_8(temp2, a[1], b[0], st);
    subtractor_8(result[2], temp1, temp2, st);
}

void vectorDot(Ctxt result[8], Ctxt a[3][8], Ctxt b[3][8], Stream *st) {
    Ctxt partial[8];
    for (int i = 0; i < 8; i++) Copy(result[i], bit_0);
    for (int i = 0; i < 3; ++i) {
        multiply_8(partial, a[i], b[i], st);
        adder_8(result, result, partial, st);
    }
}

void computeUVAndDet(Ctxt O[3][8], Ctxt D[3][8], Ctxt V0[3][8], Ctxt V1[3][8], Ctxt V2[3][8], Ctxt u_plus_v[8], Ctxt det[8], Stream *st) {
    Ctxt E1[3][8], E2[3][8];
    Ctxt P[3][8], T[3][8], Q[3][8];
    Ctxt u[8], v[8];
    ...
    vectorDot(v, D, Q, st);
    adder_8(u_plus_v, u, v, st);
}

void Add8BitEncrypted(Ctxt *sum, Ctxt *A, Stream *st) {
    int n = 8;
    Ctxt* tempSum = new Ctxt[n];
    Ctxt* tempCarry = new Ctxt[n];
    Ctxt* newCarry = new Ctxt[n];
    for (int i = 0; i < n; ++i) {
        Xor(tempSum[i], A[i], A[i + 8], st[i]);
        And(tempCarry[i], A[i], A[i + 8], st[i]);
    }
    Synchronize();
...
void server(int shares, uint8_t kNumSMs, int idx, Ctxt** answers, Ctxt* final_answer, Stream** st) {
    while(1) {
        cout << 1 << endl;
...
void setup(uint8_t kNumSMs, Ctxt** inputs, Ptxt** pt, Stream** st, int idx) {
    cudaSetDevice(idx);
    Initialize(bk);
...
int main(){
    ...
    Stream* st;
    Ptxt O_p[3][8];
    Ptxt D_p[3][8], V0_p[3][8], V1_p[3][8], V2_p[3][8];
    Ctxt u_plus_v[8];
    Ctxt det[8];
    SetSeed();
    PriKeyGen(pri_key);
    ...
    Ctxt O[3][8];
    Ctxt D[3][8], V0[3][8], V1[3][8], V2[3][8];
    ...
    for (int i = 1; i < (num_threads+1); i++) {
        if (i != 0) {
            ...
            adder_8(det, O[0], D[0], st);
            t2 = high_resolution_clock::now();

            //multiply_8(u_plus_v, O[0], D[0], st);
    ...
    Ptxt* recovered_u_plus_v = new Ptxt[8];
    Ptxt* recovered_det = new Ptxt[8];
